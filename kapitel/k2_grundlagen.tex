...
Um Domänenmodelle klarer zu gestalten, kann man bestimmte Regeln für die Modellierung einführen. Eine Regel ist zum Beispiel, einfache Beziehungen zwischen Elementen zu nutzen, um klarer zu zeigen, wie Microservices Daten austauschen. Eine andere Möglichkeit ist, bestimmte Muster aus dem Domain-Driven Design zu verwenden, die als Basis für die Schnittstellen der Microservices dienen. Es ist auch hilfreich, genau festzulegen, welche Datenarten von Methoden zurückgegeben werden und einfache, gemeinsame Modelle zu nutzen, die das Domänenmodell gut darstellen. Diese Methoden sollen den Prozess der Modellierung genauer und einheitlicher machen.

Mit Richtlinien für die autonome Domänenmodellierung kann schließlich die Unabhängigkeit und Konsistenz der Modelle gewährleistet werden. Ziel ist es, insbesondere im Kontext von Teams, die gemeinsam auf das Modell zugreifen, einen ausgewogenen Umgang mit dem Modellzugriff zu finden. Gleichzeitig sollen Änderungen so gesteuert werden, dass die Zusammenarbeit gefördert und ungewollte Abhängigkeiten zwischen Services reduziert werden \cite{rademacher}.
