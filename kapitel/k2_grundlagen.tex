...
Die Domäne einer Software bezeichnet das Themengebiet oder den Anwendungsbereich, in dem das Programm eingesetzt wird \cite[S.2]{evans} \cite[S.307]{posch}
...
Modellieren ist eine grundlegende Methode der Wissenschaft um mit komplexen Systemen umzugehen. Eine Abstraktion eines solchen Systems, die es ermöglicht, Fragestellungen dazu zu beantworten, wird als Modell bezeichnet \cite{brugge}.
...
Schichtenarchitekturen helfen die Komplexität von Software besser zu beherschen. Jedes Element einer Schicht hängt nur von Elementen derselben oder einer tieferen Schicht ab. Beim DDD wird die Geschäftslogik in eine eigene Schicht, die \glqq Domain Layer\grqq, gelegt. Dies sorgt für eine enge Verbindung zum Modell, wie es das MDD vorsieht. Mit Mustern wie dem Model-View-Controller kann man dann diese Schicht nutzen \cite[S. 69 - 73]{evans}.
%TODO Definition MVC
%TODO Zusätzliche Definition Schichtenarchitektur
%TODO Klarstellen, Modell vs Technische Umsetzung
...
Eine \glqq Factory\grqq{} erstellt Objekte. Dies ist nützlich, wenn das Erstellen eines Objekts mehr als nur das Ausführen eines Konstruktors beinhaltet, da manchmal besondere Reglen oder Logik benötigt werden. Zwei grundlegende Anforderungen an einer Factory sind:

%TODO Zweiten Punkt nochmal nachlesen
\begin{itemize}
...
Auch wenn eine Factory dafür sorgt, dass ein Objekt richtig erstellt wird, kann sie Überprüfungen an andere Teile auslagern. Es gibt weiterhin Factories, die bestehende Objekte wiederherstellen. Dabei gibt es Unterschiede, z.B. weisen sie keine neuen IDs zu und sie müssen Verletzungen von Regeln anders behandeln \cite[S. 136 - 146]{evans} \cite[S.77]{daschner}.
...
\begin{center}
\includegraphics[height=6cm]{bilder/k2/k2_soa_vs_ms.png}
...
Die Dienste können weiterhin unabhängig voneinander überarbeitet und deployt werden, ohne Beeinflussung des gesamten Systems. Build und Deployment sind dabei automatisiert. \cite[S. 2, 31-37]{wolff} \cite[S.28]{newman}.
...
\subsubsection{Model Driven SOA}
...
\subsubsection{Sun Approach}

Modellierungsphase: In dieser Phase wird VxBPMN4MS verwendet, um Geschäftsprozesse mit Variabilität zu modellieren. Dies führt zur Erstellung von variablen Geschäftsprozessmodellen und Prozesskonfigurationsdateien. Der Ansatz koordiniert Mikroservices und ihre Operationen, um spezifische Anforderungen zu erfüllen und bildet so die entsprechenden Geschäftsprozesse. Geschäftsprozesse mit ähnlichen Regeln oder Zielen werden zusammengefasst. Innerhalb dieser Gruppen werden Ähnlichkeiten und Unterschiede analysiert, um Variationspunkte, Varianten und die Beziehungen zwischen diesen Varianten zu identifizieren.

Phase der Servicezusammensetzung und -bereitstellung: In dieser Phase werden Mikroservice-Kompositionsframeworks erstellt, die eine Variabilitätsfähige Infrastruktur integrieren. Die von VxBPMN4MS erstellten Geschäftsprozessmodelle werden automatisch in diese Frameworks umgewandelt. Diese Frameworks implementieren die allgemeine Geschäftslogik und binden variabilitätsunterstützte Stubs für verschiedene Prozessvarianten ein. Zusätzliche Geschäftslogik und Projektdetails werden hinzugefügt, um die Mikroservice-Kompositionsspezifikation zu vervollständigen. Diese Spezifikation, zusammen mit den Mikroservices und einem externen Paket, wird dann in den vorgesehenen Umgebungen bereitgestellt.

Ausführungsphase: Während dieser Phase wird die bereitgestellte Mikroservice-Komposition ausgeführt, und Geschäftsprozessinstanzen werden in Echtzeit basierend auf der Prozesskonfigurationsdatei abgeleitet. Die Datei ermöglicht die Auswahl eines Variationskonfigurationsschemas entsprechend den Anforderungen oder Präferenzen. Dieses Schema spezifiziert Varianten für jeden Variationspunkt in der Servicekomposition, was zu einer Laufzeit-Geschäftsprozessinstanz führt. Wenn ein Variationspunkt während der Prozessausführung erreicht wird, werden die festgelegten Varianten (alternative Operationen des Mikroservices) dynamisch aufgerufen, und der Prozess setzt sich fort, bis alle Aktivitäten in der Instanz abgeschlossen sind.

\cite{sun}

\subsection{Herausforderungen beim Domain-Driven Microservice Design:}
...
\paragraph{Zwischenmodelle und ihre technischen Eigenschaften}

Zwischenmodelle spielen eine wichtige Rolle, indem sie eine Verbindung zwischen Domänenmodellen und Mikroservice-Architektur herstellen. Die bereitgestellten Schnittstellen werden durch UML-Lollipops symbolisiert, welche die Dienste eines Mikroservices in Bezug auf seinen Domänenkontext darstellen. Erforderliche Schnittstellen hingegen zeigen die Dienste auf, die ein Mikroservice von anderen Kontexten benötigt. Zusätzlich wird ein Bereitstellungsmodell vorgestellt, das verdeutlicht, wie jeder abgegrenzte Kontext im Domänenmodell in Mikroservice-Artefakte innerhalb einer Containerumgebung übersetzt wird. Dieses Modell veranschaulicht die Interaktion von Mikroservices, einschließlich ihrer Kommunikationswege und -protokolle, wie REST und JSON, sowie wichtiger Infrastrukturkomponenten wie Serviceerkennung und API-Gateways.

\paragraph{Verringerung der Informalität von Domänenmodellen durch Modellierungskonventionen}

Um die Informalität in Domänenmodellen zu adressieren, können Modellierungskonventionen verwendet werden. Eine wichtige Konvention ist die Vermeidung binärer Assoziationen, um Modellierern zu helfen, klar zu definieren, welcher Mikroservice Daten von einem anderen anfordert, was zu einer verbesserten Klarheit in der Serviceinteraktion führt. Zusätzlich wird die frühzeitige Festlegung von Domain-Driven Design (DDD)-Mustern nahegelegt, die die Basis für Mikroservice-Schnittstellen bilden, einschließlich der Definition der Umwandlung von Repositories und Services in Schnittstellen. Auch die Wichtigkeit der expliziten Modellierung von Rückgabetypen von Methoden und der Einsatz von gemeinsam genutzten Modellen wird hervorgehoben. Diese Modelle könnten die Entitäten des Domänenmodells reflektieren, aber für eine höhere Effizienz eine vereinfachte Struktur aufweisen. Das Hauptziel dieser Konventionen ist es, den Modellierungsprozess genauer und konsistenter zu machen.

\paragraph{Richtlinien für autonomes Domänenmodellieren}
Richtlinien werden verwendet, um die Handhabung von Domänenmodellen in Szenarien mit selbstständigen, verteilten Serviceteams effektiv zu gestalten. Ein bewährter Ansatz ist die Beschränkung des Zugriffs auf die für jedes Team relevanten Segmente des Domänenmodells. Dies gewährleistet, dass Teams die notwendigen Informationen zu ihren Diensten und den damit verbundenen Konzepten aus anderen Bereichen erhalten. Diese Vorgehensweise zielt darauf ab, die Verknüpfung zwischen den Services zu minimieren und erfordert eine umsichtige Verwaltung des Modells, um die Konsistenz im gesamten Domänenmodell sicherzustellen. Ferner werden Empfehlungen für erlaubte Modifikationen innerhalb festgelegter Kontexte oder in Bezug auf die Domänenkonzepte anderer Teams erörtert. Obwohl Änderungen zur Verbesserung oder Aktualisierung des Modells nützlich sein können, erfordern sie eine koordinierte Abstimmung unter den beteiligten Teams und die Berücksichtigung neuer Abhängigkeiten, die aus diesen Änderungen resultieren könnten. Das Hauptziel dieser Richtlinien ist es, einen ausgewogenen Ansatz für den Modellzugriff und dessen Modifikationen zu finden, um gleichzeitig die Zusammenarbeit zu fördern und unbeabsichtigte Abhängigkeiten zwischen den Services zu minimieren.
...
VxBPMN4MS \cite{sun} & 
\begin{itemize}[leftmargin=*, nosep, before=\vspace{-0.6\baselineskip}, after=\vspace{-\baselineskip}]
    \item Modellieren von Geschäftsprozessen mit eingebauter Variabilität für flexible und anpassbare Microservice-Architekturen
    \item Definiert strukturierten Entwicklungs- und Ausführungsprozess durch Phasen, die ein Management und Anpassungsfähigkeit der Microservices gewährleisten
    \item Werkzeuge für das visuelle Modellieren und Implementieren einer Infrastruktur
\end{itemize} \\
\hline
